...
void push_back(int*& arr, int num) {
	// функция расширения массива (добавление в конец алалогично list.append() в питоне)
	int len = arr[0];
	arr[0]++;
...
void task_3(int* data) {
	...
	for (int i = 1; i <= ctn; i++) {
		for (int j = 1; j <= ctn - 1 - i; j++) {
			int a = 0, b = 0, ctn = 0;
			// get summ data[j]
			// получаем сумму цифр числа data[j], стоящих на чётных местах
...
int main()
{
	...
	while (run) {
		...
		{
		case 1:
			// input nums получение номера меню
	...
	delete[] arr;  // удаление ненужного массива
	return 0;
}
